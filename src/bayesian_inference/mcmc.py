...
def run_mcmc(config, closure_index=-1):
    ...
    mcmc_sampler_name = emulation_config.mcmc_config.get("mcmc_sampler", "emcee")
    if mcmc_sampler_name == "emcee":
        _run_using_emcee(
            config,
            emulation_config,
            emulation_results,
        ...
        msg = f"Invalid MCMC sampler: {mcmc_sampler_name}"
...
class PocoMCConfig(common_base.CommonBase):
    ...
    def __init__(self, analysis_name="", parameterization="", analysis_config="", config_file="",
        ...
        try:
            mcmc_configuration = analysis_config["parameters"]["pocoMC"]
        ...
        # Validation
        if self.n_effective > self.n_active:
        ...
        self.sampler_type = mcmc_configuration.get("sampler_type", "pcn")
